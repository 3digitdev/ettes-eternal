# Spell Summary Table

| Spell Name         | Level | Description                                          |
| ------------------ | ----- | ---------------------------------------------------- |
| Alter Self         | 2     | Minor physical transformation                        |
| Elemental Burst    | 2     | Choose damage type with type-specific effects        |
| Energy Orb         | 2     | Floating orb that continuously attacks target        |
| Frost Nova         | 2     | Ice explosion centered on caster with slowing effect |
| Levitate           | 2     | Float target in air with horizontal movement         |
| Minor Invisibility | 2     | Short-duration invisibility                          |
| Misty Step         | 2     | Short-range teleportation                            |
| Scorching Ray      | 2     | Line of searing fire                                 |
| Spider Climb       | 2     | Walk on walls and ceilings                           |
| Thunder Wave       | 2     | Sonic burst that damages, pushes, and deafens        |

---
# Spell Descriptions
## Alter Self
**Level:** 2  
**Mana Cost:** 2  
**Range:** Self  
**Target:** Self  
**Duration:** Sustained

**Effect:** Minor physical transformation. Change height by up to 1 foot, alter facial features, change hair/eye color, or hide small features.

**Critical Effect:** Changes are permanent until you choose to revert them (no concentration required).

|Class Tier|Additional Effect|
|---|---|
|Apprentice|Basic alterations|
|Journeyman|Basic alterations|
|Expert|Can change apparent ancestry|
|Master|Can change apparent ancestry and gender|
|Legendary|Can mimic specific individuals you've seen|

---
## Elemental Burst
**Level:** 2  
**Mana Cost:** 2  
**Range:** 8 hexes  
**Target:** Blast 2  
**Duration:** Instant

**Effect:** Choose Fire, Ice, or Shock when casting. Creates moderate elemental damage with type-specific effects.

|Class Tier|Damage|Additional Effect|
|---|---|---|
|Apprentice|1d6 + Spellcasting modifier|Fire ignites flammables; Ice creates difficult terrain 3 rounds; Shock causes targets to lose 1 AP next turn|
|Journeyman|2d6 + Spellcasting modifier|Fire ignites flammables; Ice creates difficult terrain 3 rounds; Shock causes targets to lose 1 AP next turn|
|Expert|2d6 + Spellcasting modifier|Fire ignites flammables; Ice creates difficult terrain 4 rounds; Shock causes targets to lose 2 AP next turn|
|Master|3d6 + Spellcasting modifier|Fire ignites flammables; Ice creates difficult terrain 4 rounds; Shock causes targets to lose 2 AP next turn|
|Legendary|4d6 + Spellcasting modifier|Fire ignites flammables; Ice creates difficult terrain 5 rounds; Shock causes targets to lose 3 AP next turn|

---
## Energy Orb
**Level:** 2  
**Mana Cost:** 2  
**Range:** 10 hexes  
**Target:** One creature  
**Duration:** Sustained

**Effect:** Create a floating orb of energy that follows and harasses a target. Orb attacks automatically at end of each of your turns with no action required.

|Class Tier|Damage|Additional Effect|
|---|---|---|
|Apprentice|1d8 + Spellcasting modifier|Orb lasts 3 rounds|
|Journeyman|2d8 + Spellcasting modifier|Orb lasts 3 rounds|
|Expert|2d8 + Spellcasting modifier|Orb lasts 4 rounds|
|Master|3d8 + Spellcasting modifier|Orb lasts 4 rounds|
|Legendary|4d8 + Spellcasting modifier|Orb lasts 5 rounds|

---
## Frost Nova
**Level:** 2  
**Mana Cost:** 2  
**Range:** N/A  
**Target:** Blast 3 centered on caster  
**Duration:** Instant

**Effect:** Ice explodes outward from your position, potentially slowing enemies. Targets make Endurance Opposed Check or are Slowed for a number of rounds.

|Class Tier|Damage|Slowed Duration|
|---|---|---|
|Apprentice|1d8 + Spellcasting modifier|2 rounds|
|Journeyman|2d8 + Spellcasting modifier|2 rounds|
|Expert|2d8 + Spellcasting modifier|3 rounds|
|Master|3d8 + Spellcasting modifier|3 rounds|
|Legendary|4d8 + Spellcasting modifier|4 rounds|

---
## Levitate
**Level:** 2  
**Mana Cost:** 2  
**Range:** 6 hexes  
**Target:** One creature or object  
**Duration:** Sustained

**Effect:** Target hovers above ground and can move horizontally at half speed. Unwilling targets get Willpower Opposed Check.

**Critical Effect:** Target can fly at full speed instead of hovering at half speed.

|Class Tier|Height|Additional Effect|
|---|---|---|
|Apprentice|2 hexes|-|
|Journeyman|2 hexes|-|
|Expert|3 hexes|Can affect 1 additional target|
|Master|3 hexes|Can affect 1 additional target|
|Legendary|4 hexes|Can affect 2 additional targets|

---
## Minor Invisibility
**Level:** 2  
**Mana Cost:** 2  
**Range:** Touch  
**Target:** One creature  
**Duration:** 3 rounds

**Effect:** Target becomes invisible for duration or until they attack or cast a damaging spell.

**Critical Effect:** Duration doubled.

|Class Tier|Duration|
|---|---|
|Apprentice|3 rounds|
|Journeyman|3 rounds|
|Expert|4 rounds|
|Master|4 rounds|
|Legendary|5 rounds|

---
## Misty Step
**Level:** 2  
**Mana Cost:** 2  
**Range:** Self  
**Target:** Self  
**Duration:** Instant

**Effect:** Teleport to any location you can see. Does not provoke Reaction Attacks.

**Critical Effect:** Can teleport one willing adjacent ally with you.

|Class Tier|Range|
|---|---|
|Apprentice|3 hexes|
|Journeyman|4 hexes|
|Expert|5 hexes|
|Master|6 hexes|
|Legendary|7 hexes|

---
## Scorching Ray
**Level:** 2  
**Mana Cost:** 2  
**Range:** N/A  
**Target:** Line 8  
**Duration:** Instant

**Effect:** A line of searing fire streaks from your fingertips.

|Class Tier|Damage|
|---|---|
|Apprentice|1d8 + Spellcasting modifier|
|Journeyman|2d8 + Spellcasting modifier|
|Expert|2d8 + Spellcasting modifier|
|Master|3d8 + Spellcasting modifier|
|Legendary|4d8 + Spellcasting modifier|

---
## Frost Nova
**Level:** 2  
**Mana Cost:** 2  
**Range:** Self  
**Target:** Blast 3 centered on caster  
**Duration:** Instant

**Effect:** Ice explodes outward from your position, potentially slowing enemies.

|Class Tier|Damage|Additional Effect|
|---|---|---|
|Apprentice|1d8 + Spellcasting modifier|Targets make Endurance Opposed Check or Slowed 2 rounds|
|Journeyman|2d8 + Spellcasting modifier|Targets make Endurance Opposed Check or Slowed 2 rounds|
|Expert|2d8 + Spellcasting modifier|Targets make Endurance Opposed Check or Slowed 3 rounds|
|Master|3d8 + Spellcasting modifier|Targets make Endurance Opposed Check or Slowed 3 rounds|
|Legendary|4d8 + Spellcasting modifier|Targets make Endurance Opposed Check or Slowed 4 rounds|

---
## Energy Orb
**Level:** 2  
**Mana Cost:** 2  
**Range:** 10 hexes  
**Target:** One creature  
**Duration:** Sustained

**Effect:** Create a floating orb of energy that follows and harasses a target. Orb attacks automatically at end of each of your turns with no action required.

|Class Tier|Damage|Additional Effect|
|---|---|---|
|Apprentice|1d8 + Spellcasting modifier|Orb lasts 3 rounds|
|Journeyman|2d8 + Spellcasting modifier|Orb lasts 3 rounds|
|Expert|2d8 + Spellcasting modifier|Orb lasts 4 rounds|
|Master|3d8 + Spellcasting modifier|Orb lasts 4 rounds|
|Legendary|4d8 + Spellcasting modifier|Orb lasts 5 rounds|
## Scorching Ray
**Level:** 2  
**Mana Cost:** 2  
**Range:** Self  
**Target:** Line 8  
**Duration:** Instant

**Effect:** A line of searing fire streaks from your fingertips.

|Class Tier|Damage|
|---|---|
|Apprentice|1d8 + Spellcasting modifier|
|Journeyman|2d8 + Spellcasting modifier|
|Expert|2d8 + Spellcasting modifier|
|Master|3d8 + Spellcasting modifier|
|Legendary|4d8 + Spellcasting modifier|

---
## Spider Climb
**Level:** 2  
**Mana Cost:** 2  
**Range:** Touch  
**Target:** One creature  
**Duration:** Sustained

**Effect:** Target can walk on walls and ceilings at normal movement speed.

**Critical Effect:** Target leaves no tracks and cannot be detected by tremorsense or similar ground-based detection.

|Class Tier|Additional Effect|
|---|---|
|Apprentice|1 target|
|Journeyman|1 target|
|Expert|Can affect 1 additional target|
|Master|Can affect 1 additional target|
|Legendary|Can affect 2 additional targets|

---
## Thunder Wave
**Level:** 2  
**Mana Cost:** 2  
**Range:** N/A  
**Target:** Cone 3  
**Duration:** Instant

**Effect:** Sonic energy erupts from you in all directions.

|Class Tier|Damage|Additional Effect|
|---|---|---|
|Apprentice|1d8 + Spellcasting modifier|Targets make Endurance Opposed Check or pushed back 2 hexes and deafened 3 rounds|
|Journeyman|2d8 + Spellcasting modifier|Targets make Endurance Opposed Check or pushed back 2 hexes and deafened 3 rounds|
|Expert|2d8 + Spellcasting modifier|Targets make Endurance Opposed Check or pushed back 3 hexes and deafened 3 rounds|
|Master|3d8 + Spellcasting modifier|Targets make Endurance Opposed Check or pushed back 3 hexes and deafened 4 rounds|
|Legendary|4d8 + Spellcasting modifier|Targets make Endurance Opposed Check or pushed back 4 hexes and deafened 4 rounds|